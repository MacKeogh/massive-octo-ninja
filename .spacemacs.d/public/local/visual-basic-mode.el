...
(if visual-basic-mode-map
  ...
  (cond (visual-basic-winemacs-p (define-key visual-basic-mode-map '(control C) 'visual-basic-start-ide))
        (visual-basic-win32-p (define-key visual-basic-mode-map (read "[?\\S-\\C-c]") 'visual-basic-start-ide)))
  (if visual-basic-xemacs-p
      (progn (define-key visual-basic-mode-map "\M-G" 'visual-basic-grep)
...
(defun visual-basic-mark-defun ()
  ...
  ;(if visual-basic-xemacs-p (zmacs-activate-region))
  )
...
changed files."
  (interactive)
  (let (file)
    (cond ((null visual-basic-ide-pathname) (error "No pathname set for Visual Basic.  See visual-basic-ide-pathname"))
          ((null (setq file (visual-basic-buffer-project-file))) (error "No project file found"))
          ;((fboundp 'win-exec) (iconify-emacs)
          ;                     (win-exec visual-basic-ide-pathname 'win-show-normal file))
          ((fboundp 'start-process) (iconify-frame (selected-frame))
                                    (start-process "*VisualBasic*" nil visual-basic-ide-pathname file))
          (t (error "No way to spawn process!")))))
      ...
      (cond ((or (looking-at visual-basic-defun-start-regexp)
           ...
           (t
              ...
              (while (looking-at visual-basic-label-regexp)
                  ...
                  (cond ((looking-at visual-basic-continuation-regexp) (visual-basic-find-original-statement)
                  ;; Indent continuation line under matching open paren,
                  ;; or else one word in.
                          (let* ((orig-stmt (point))
                                 (matching-open-paren (condition-case ()
                                                        (save-mark-and-excursion
                                                        ...
                                                        (if (<= orig-stmt (point))
                                                          (current-column)))
                                                          (error nil))))
                             (cond (matching-open-paren (1+ matching-open-paren))
                                   (t
                             ...
                             (let ((indent (current-indentation)))
                               ...
                               (cond ((looking-at visual-basic-defun-start-regexp)
                                     ...
                                     ((or (looking-at visual-basic-do-regexp)
                                        ...
                                        (+ indent visual-basic-mode-indent))
                                     (t ;; By default, just copy indent from prev line.
                                        indent)))))))))
  )

(defun visual-basic-indent-to-column (col)
  (let* ((bol (save-mark-and-excursion
                (beginning-of-line)
                (point)))
         (point-in-whitespace (<= (point) (+ bol (current-indentation))))
         (blank-line-p (save-mark-and-excursion
                           (beginning-of-line)
  ...
  (cond (blank-line-p (end-of-line))
        (point-in-whitespace (back-to-indentation)))))
...
In Abbrev mode, any abbrev before point will be expanded."
  (interactive)
  (let ((pps-list (parse-partial-sexp (save-mark-and-excursion
                                        (beginning-of-line)
                                        (point))
                                      (point))))
